...
const updateColumnsMutate = vi.fn();

vi.mock('@/lib/hooks/use-boards', () => ({
  useUpdateBoard: () => ({ mutate: vi.fn(), isPending: false }),
  useUpdateBoardColumns: () => ({ mutate: updateColumnsMutate, isPending: false }),
}));
...
beforeEach(() => updateColumnsMutate.mockClear());
...
describe('BoardSettingsDialog', () => {
  ...
  it('drops empty columns from the save payload', async () => {
    ...
    expect(updateColumnsMutate).toHaveBeenCalledTimes(1);
