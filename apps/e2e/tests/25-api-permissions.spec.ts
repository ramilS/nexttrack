...
test.describe('API Boundary: Permission Matrix', () => {
  ...
  test('project member can create comments on issues', async ({ request }) => {
    ...
    // The seed populates this project — assert (don't silently skip) so a broken
    // seed/visibility fails loudly instead of disabling the permission check.
  ...
  test('non-member cannot access private project', async () => {
    ...
    // Admin must be able to create the private-project fixture — assert rather
    // than skip, so a regression in project creation surfaces here.
