...
describe('AuthController', () => {
  ...
  describe('logout', () => {
    it('forwards the refresh cookie to authService.logout and clears tokens', async () => {
      const req = mockRequest({
        cookies: { refresh_token: 'refresh-jwt-cookie' },
      });
      const res = mockResponse();

      await controller.logout('user-1', req, res);

      // Verify/decode now lives in AuthService — the controller just forwards.
    ...
    it('forwards undefined and clears tokens when no cookie is present', async () => {
