...
// Sentinel transaction handle the mocked TransactionService hands to callbacks;
// lets the assertions verify the tx is threaded into every write.
const tx = {} as never;
...
describe('TimeLogsService', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: ActivitiesService, useValue: activitiesService },
        {
          ...
          useValue: { run: jest.fn((cb: (t: never) => unknown) => cb(tx)) },
        },
        { provide: IndexerHooksService, useValue: indexerHooks },
        {
          provide: BackgroundTasks,
          useValue: {
            run: jest.fn((task: () => Promise<unknown>, onError: (e: Error) => void) => {
  ...
  describe('create', () => {
    it('creates a time log and recalculates spent', async () => {
      ...
      expect(issuesRepo.updateSpent).toHaveBeenCalledWith('issue-1', 60, tx);
      expect(activitiesService.recordOne).toHaveBeenCalled();
      expect(indexerHooks.onIssueChanged).toHaveBeenCalledWith('issue-1', 'time_log');
    });
  ...
  describe('update', () => {
    it('updates a log when called by its owner', async () => {
      ...
      expect(issuesRepo.updateSpent).toHaveBeenCalledWith('issue-1', 90, tx);
    });
  ...
  describe('softDelete', () => {
    it('soft-deletes the log and recalculates spent', async () => {
      ...
      expect(timeLogsRepo.softDelete).toHaveBeenCalledWith('log-1', 'user-1', tx);
      expect(issuesRepo.updateSpent).toHaveBeenCalledWith('issue-1', 0, tx);
    });
