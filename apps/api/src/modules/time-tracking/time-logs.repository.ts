...
export class TimeLogsRepository {
  ...
  async create(input: TimeLogCreateInput): Promise<TimeLog> {
    const row = await this.prisma.timeLog.create({
      data: {
  ...
  async update(logId: string, patch: TimeLogPatch): Promise<TimeLog> {
    ...
    const row = await this.prisma.timeLog.update({
      where: { id: logId },
      data,
      include: { user: { select: USER_SELECT } },
    });
  ...
  async softDelete(logId: string, deletedBy: string): Promise<void> {
  ...
  async sumDurationForIssue(issueId: string): Promise<number> {
    const agg = await this.prisma.timeLog.aggregate({
