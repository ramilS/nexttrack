...
describe('TimeReportsService', () => {
  ...
  beforeEach(async () => {
    timeLogsRepo = {
      ...
      countReportLogs: jest.fn().mockResolvedValue(0),
      countUserReportLogs: jest.fn().mockResolvedValue(0),
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: timeTrackingConfig.KEY,
          useValue: { maxReportRows: MAX_REPORT_ROWS },
        },
      ],
    }).compile();
  ...
  describe('report row-limit guard', () => {
    it('rejects getTimeReport without fetching when the match count exceeds the cap', async () => {
      timeLogsRepo.countReportLogs.mockResolvedValue(MAX_REPORT_ROWS + 1);

      await expect(service.getTimeReport('project-1', baseDto)).rejects.toMatchObject(
        { code: ErrorCode.TIME_REPORT_RANGE_TOO_LARGE },
      );
      expect(timeLogsRepo.findReportLogs).not.toHaveBeenCalled();
    });

    it('rejects exportReport when the match count exceeds the cap', async () => {
      timeLogsRepo.countReportLogs.mockResolvedValue(MAX_REPORT_ROWS + 1);

      await expect(
        service.exportReport('project-1', baseDto, 'csv'),
      ).rejects.toMatchObject({ code: ErrorCode.TIME_REPORT_RANGE_TOO_LARGE });
      expect(timeLogsRepo.findReportLogs).not.toHaveBeenCalled();
    });

    it('rejects getUserTimeReport without fetching when the match count exceeds the cap', async () => {
      timeLogsRepo.countUserReportLogs.mockResolvedValue(MAX_REPORT_ROWS + 1);

      await expect(
        service.getUserTimeReport('user-1', {
          dateFrom: '2026-03-01',
          dateTo: '2026-03-31',
        }),
      ).rejects.toMatchObject({ code: ErrorCode.TIME_REPORT_RANGE_TOO_LARGE });
      expect(timeLogsRepo.findUserReportLogs).not.toHaveBeenCalled();
    });

    it('allows a report exactly at the cap', async () => {
      timeLogsRepo.countReportLogs.mockResolvedValue(MAX_REPORT_ROWS);
      timeLogsRepo.findReportLogs.mockResolvedValue([]);

      await expect(
        service.getTimeReport('project-1', baseDto),
      ).resolves.toBeDefined();
      expect(timeLogsRepo.findReportLogs).toHaveBeenCalled();
    });
  });
});
