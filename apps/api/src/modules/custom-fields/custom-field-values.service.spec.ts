...
describe('CustomFieldValuesService', () => {
  ...
  let activities: { recordOne: jest.Mock };
  ...
  beforeEach(async () => {
    ...
    activities = { recordOne: jest.fn().mockResolvedValue(undefined) };
  ...
  describe('setFieldValue', () => {
    ...
    it('upserts a validated value and records activity', async () => {
      ...
      expect(valuesRepo.upsert).toHaveBeenCalledWith(issueId, 'f1', 'validated');
      expect(activities.recordOne).toHaveBeenCalled();
      expect(issuesRepo.touchUpdatedAt).toHaveBeenCalledWith(issueId);
    });
    ...
    it('deletes existing value when setting null on non-required field', async () => {
      ...
      expect(valuesRepo.deleteById).toHaveBeenCalledWith('cfv-1');
    });
  ...
  describe('setInitialFieldValues', () => {
    ...
    it('validates and upserts each value through the given transaction', async () => {
      fieldsRepo.findOneInProject.mockResolvedValue(textField);
      validator.validate.mockResolvedValue('validated');
      ...
      expect(valuesRepo.upsert).toHaveBeenCalledWith(issueId, 'f1', 'validated', tx);
      expect(activities.recordOne).toHaveBeenCalledWith(
        issueId,
        userId,
        expect.anything(),
    ...
    it('skips null values for optional fields without writing', async () => {
      ...
      expect(valuesRepo.upsert).not.toHaveBeenCalled();
      expect(activities.recordOne).not.toHaveBeenCalled();
    });

    it('throws FIELD_REQUIRED for a null value on a required field', async () => {
      fieldsRepo.findOneInProject.mockResolvedValue({ ...textField, isRequired: true });
    ...
    it('propagates validation errors so the caller transaction rolls back', async () => {
      fieldsRepo.findOneInProject.mockResolvedValue(textField);
      validator.validate.mockRejectedValue(new ValidationError('bad value'));
