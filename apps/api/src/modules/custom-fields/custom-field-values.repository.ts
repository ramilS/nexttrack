...
export class CustomFieldValuesRepository {
  ...
  /**
   * Inserts the initial values for a freshly-created issue in one statement.
   * Safe to use only when the issue has no pre-existing values (the create
   * path) — there is no conflict handling, unlike {@link upsert}.
   */
  async createMany(
    issueId: string,
    ...
    await (tx ?? this.prisma).customFieldValue.createMany({
      data: values.map((v) => ({
        issueId,
        customFieldId: v.fieldId,
        value: asJson(v.value),
      })),
    });
  }

  async deleteById(id: string, tx?: Tx): Promise<void> {
    await (tx ?? this.prisma).customFieldValue.delete({ where: { id } });
  }
  ...
  async findValuesUsingOption(
    ...
    // Push the option-reference predicate into Postgres via JSON operators
    // instead of loading every value of the field and filtering in memory.
    // Single-select stores the scalar id; multi-select stores an array of ids.
    return this.prisma.customFieldValue.findMany({
      where: {
        customFieldId: fieldId,
        value: isMulti ? { array_contains: optionId } : { equals: optionId },
      },
