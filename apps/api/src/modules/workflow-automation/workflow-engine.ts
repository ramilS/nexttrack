...
export class WorkflowEngine {
  private readonly logger = new AppLogger(WorkflowEngine.name);
  ...
  async executeRules(
    ...
    if (this.executionDepth >= MAX_EXECUTION_DEPTH) {
      this.logger.warn('Workflow execution depth limit reached, skipping rules', {
    ...
    if (recentFiltered.length >= MAX_EXECUTIONS_PER_ISSUE_PER_MINUTE) {
      this.logger.warn('Workflow execution rate limit reached, skipping', {
        issueId: issueKey,
        projectId,
        limitPerMinute: MAX_EXECUTIONS_PER_ISSUE_PER_MINUTE,
      });
      return;
    }
    ...
    try {
      for (const rule of rules) {
        ...
        try {
          ...
          this.logger.log('Workflow rule executed', {
            ruleId: rule.id,
            issueId: actionContext.issueId,
            projectId,
            trigger,
          ...
          this.logger.error('Workflow rule execution failed', err, {
            ruleId: rule.id,
            issueId: actionContext.issueId,
            projectId,
            trigger,
