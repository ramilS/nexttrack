...
export class IssueLinksRepository {
  ...
  /** Dependency-cycle BFS: returns ids reachable via DEPENDS_ON from a starting set. */
  async findDependsOnTargets(sourceIssueId: string): Promise<string[]> {
    const rows = await this.prisma.issueLink.findMany({
      where: { sourceIssueId, type: IssueLinkType.DEPENDS_ON },
      select: { targetIssueId: true },
    });
    return rows.map((r) => r.targetIssueId);
  }
}
