...
describe('IssuesService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        // Real hierarchy service — resolves with the mocked IssuesRepository, so
        // the existing parent/cycle tests exercise the delegated logic unchanged.
  ...
  describe('bulkUpdate', () => {
    ...
    it('rejects a bulk assignee who is not a project member', async () => {
      issuesRepo.findManyForBulk.mockResolvedValue([{ id: 'i1', statusId: 'open' }]);
      membersRepo.isMember.mockResolvedValue(false);
      ...
      await expect(
        service.bulkUpdate(
          project,
          { issueIds: ['i1'], update: { assigneeId: 'outsider' } } as never,
          'u1',
        ),
      ).rejects.toThrow(ValidationError);
      expect(issuesRepo.bulkUpdate).not.toHaveBeenCalled();
    });

    it('rejects bulk tags that do not belong to the project', async () => {
      issuesRepo.findManyForBulk.mockResolvedValue([{ id: 'i1', statusId: 'open' }]);
      tagsRepo.countInProject.mockResolvedValue(0); // 0 of 1 tag in project

      await expect(
        service.bulkUpdate(
          project,
          { issueIds: ['i1'], update: { tagIds: ['foreign-tag'] } } as never,
          'u1',
        ),
      ).rejects.toThrow(ValidationError);
      expect(issuesRepo.bulkUpdate).not.toHaveBeenCalled();
    ...
    it('publishes one issue.updated event per affected issue', async () => {
      issuesRepo.findManyForBulk.mockResolvedValue([
        { id: 'i1', statusId: 'open' },
        { id: 'i2', statusId: 'open' },
      ]);
      ...
      await service.bulkUpdate(
        project,
        { issueIds: ['i1', 'i2'], update: { priority: Priority.HIGH } } as never,
        'u1',
      );

      const updatedEvents = domainEvents.publish.mock.calls.filter(
        ([evt]) => evt.eventType === 'issue.updated',
      );
      expect(updatedEvents).toHaveLength(2);
      expect(updatedEvents.map(([evt]) => evt.aggregateId)).toEqual(['i1', 'i2']);
    });
  });
