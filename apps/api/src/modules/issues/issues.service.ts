...
export class IssuesService {
  ...
  async create(
    ...
    if (dto.parentId) {
      await this.assertValidParent(project.id, dto.parentId);
    }
  ...
  async update(
    ...
    // buildActivities is typed for the full Prisma Issue but only reads the
    // change-tracked fields, all of which IssueUpdateContext provides.
    const activities = buildActivities(
      issue as Parameters<typeof buildActivities>[0],
      { ...dto, ...updateData },
  ...
  private async resolveUpdatePatch(
    ...
    if (dto.parentId !== undefined) {
      if (dto.parentId) {
        await this.assertValidParent(project.id, dto.parentId);
        await this.checkParentCycle(issue.id, dto.parentId);
      }
  ...
  async bulkUpdate(
    ...
    if (dto.update.statusId) {
      const workflow = await this.requireDefaultWorkflow(project.id);
      const targetStatus = workflow.statuses.find((s) => s.id === dto.update.statusId);
      if (!targetStatus) {
        throw new ValidationError(
    ...
    await this.txService.run(async (tx) => {
      ...
      if (dto.update.tagIds !== undefined) {
        await this.tagsRepo.replaceIssueLinksBulk(validIssueIds, dto.update.tagIds, tx);
      }
    });
  ...
  private async assertValidParent(projectId: string, parentId: string): Promise<void> {
  ...
  private async checkParentCycle(issueId: string, newParentId: string): Promise<void> {
