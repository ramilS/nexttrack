...
export class IssueEventsListener {
  private readonly logger = new AppLogger(IssueEventsListener.name);
  ...
  async handleIssueCreated(
    ...
    this.logger.log('Handling issue.created', {
    ...
    await this.runWorkflowRules(
      ...
      this.logger.error('Workflow rules (ON_CREATE) failed', err, {
        eventId: event.eventId,
        issueId: event.issueId,
        projectId: event.projectId,
      }),
  ...
  async handleIssueUpdated(
    ...
    this.logger.log('Handling issue.updated', {
      eventId: event.eventId,
      issueId: event.issueId,
      projectId: event.projectId,
      activityCount: event.activities.length,
      statusChanged: event.changes.statusId !== undefined,
      assigneeChanged: event.changes.assigneeId !== undefined,
    });
    ...
    if (event.changes.statusId && event.changes.statusId !== event.previous.statusId) {
      await this.runWorkflowRules(
        ...
        this.logger.error('Workflow rules (ON_STATUS_CHANGE) failed', err, {
          eventId: event.eventId,
          issueId: event.issueId,
          projectId: event.projectId,
          from: event.previous.statusId,
          to: event.changes.statusId,
        }),
      );
