...
export class IssuesRepository {
  ...
  async findIssueRef(issueId: string): Promise<IssueRef | null> {
    ...
    return { ...rest, projectKey: project.key };
  }
  ...
  async updateSpent(issueId: string, spent: number, tx?: Tx): Promise<void> {
   ...
   */
  async touchUpdatedAt(issueId: string, tx?: Tx): Promise<void> {
    await this.db(tx).issue.update({
      where: { id: issueId },
  ...
  // Deliberately unbounded (query-perf rule #5 "show-all"): a board renders every
  // active issue at once. where + orderBy are covered by
  // @@index([projectId, deletedAt, priority, createdAt]).
    ...
    if (tagIds !== undefined) {
      ...
      if (tagIds.length > 0) {
        await tx.issueTag.createMany({
          data: tagIds.map((tagId) => ({ issueId, tagId, projectId })),
        });
      }
    }
