...
export class SsoAccountService {
  private readonly logger = new AppLogger(SsoAccountService.name);
  ...
  async connectToExistingAccount(
    ...
    this.logger.log('SSO provider connected to account', { userId, providerId });
  ...
  async disconnect(userId: string, providerId: string) {
    ...
    this.logger.log('SSO provider disconnected from account', { userId, providerId });
  }
