...
export class DocSuggestionService {
  private readonly logger = new AppLogger(DocSuggestionService.name);
  ...
  async suggest(
    ...
    if (!parsed.shouldUpdate) {
      this.logger.debug('AI decided no doc update needed (shouldUpdate=false)', {
  ...
  private parseContent(json: string, issueKey: string): TiptapDoc | null {
    try {
      ...
      this.logger.warn('AI returned invalid Tiptap content; skipping suggestion', {
        issueKey,
      });
      return null;
    }
