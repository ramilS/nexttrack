...
export class AiDocsListener {
  private readonly logger = new AppLogger(AiDocsListener.name);
  ...
  async handleIssueUpdated(
    ...
    this.logger.log('Queued AI doc-suggestion for resolved issue', {
      sourceIssueId: event.issueId,
      projectId: event.projectId,
    });
  }
  ...
  private async resolveDocIssue(
    ...
    if (newStatus && this.isRejectionStatus(newStatus.name)) {
      ...
      this.logger.log('Doc-update issue cancelled; proposal rejected', {
        docIssueId: event.issueId,
    ...
    if (!project) {
      this.logger.warn('Doc-update issue resolved but its project is gone; skipping apply', {
        docIssueId: event.issueId,
        proposalId: proposal.id,
    ...
    this.logger.log('Doc-update issue resolved', {
      docIssueId: event.issueId,
      proposalId: proposal.id,
