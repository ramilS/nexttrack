...
/**
 * The change-tracked fields `buildActivities` compares against the new data.
 * A `Pick` rather than the full `Issue` so both the single-update path
 * (IssueUpdateContext) and the bulk path (a lighter projection) satisfy it
 * without a cast.
 */
export type ActivityComparable = Pick<
  PrismaIssue,
  'title' | 'statusId' | 'assigneeId' | 'priority' | 'type' | 'estimate' | 'dueDate' | 'parentId'
>;
