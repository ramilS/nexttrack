...
export class MailService {
  ...
  async sendInvite(
    ...
    try {
      ...
      this.logger.error('Invite email send failed', error, {
        template: 'invite',
      });
      throw error;
    }
  ...
  async sendNotificationEmail(
    ...
    try {
      ...
      this.logger.error('Notification email send failed', error, {
        notificationType: data.type,
        template: templateName,
      });
      throw error;
  ...
  async sendDigestEmail(
    ...
    try {
      ...
      this.logger.log('Digest email sent', {
        template: 'digest',
        notificationCount: data.notifications.length,
        messageId: info.messageId,
      });
    } catch (error) {
      this.logger.error('Digest email send failed', error, {
        template: 'digest',
        notificationCount: data.notifications.length,
      });
      throw error;
