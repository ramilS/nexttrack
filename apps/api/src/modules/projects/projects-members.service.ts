...
export class ProjectsMembersService {
  private readonly logger = new AppLogger(ProjectsMembersService.name);
  ...
  async addMember(
    ...
    const [userExists, role, alreadyMember] = await Promise.all([
      this.usersRepo.existsActiveById(dto.userId),
      this.rolesRepo.findById(dto.roleId),
      this.membersRepo.isMember(dto.userId, project.id),
    ]);
    ...
    this.logger.log('Project member added', {
      projectId: project.id,
      memberId: dto.userId,
      roleId: dto.roleId,
    });
  ...
  async updateRole(
    ...
    await this.permissionsCache.invalidateAll();
    this.logger.log('Project member role changed', {
      projectId: project.id,
      memberId: userId,
  ...
  async removeMember(project: ProjectEntity, userId: string): Promise<void> {
    ...
    await this.permissionsCache.invalidateAll();
    this.logger.log('Project member removed', {
      projectId: project.id,
      memberId: userId,
