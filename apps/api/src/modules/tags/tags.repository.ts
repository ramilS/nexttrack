...
export class TagsRepository {
   ...
   * `projectId` is the issues' (and tags') project — it is part of the
   * issue_tags composite FK that enforces tenant isolation.
   */
    ...
    if (tagIds.length > 0) {
      await this.db(tx).issueTag.createMany({
        data: issueIds.flatMap((issueId) =>
          tagIds.map((tagId) => ({ issueId, tagId, projectId })),
        ),
      });
    }
  }
