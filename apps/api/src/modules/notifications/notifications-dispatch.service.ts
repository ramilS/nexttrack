...
export class NotificationsDispatchService {
  private readonly logger = new AppLogger(NotificationsDispatchService.name);
  ...
  async dispatch(input: DispatchNotificationInput) {
    ...
    this.logger.log('Notification dispatched', {
      ...
      outboxEventCount,
      webhookCount: external.webhooks.length,
      telegram: external.telegram !== null,
    });
  ...
  private async persistInAppNotifications(
    ...
    await this.notificationsRepo.incrementGroupedNotifications(
      existing.map((n) => n.id),
      input.payload,
      tx,
    );
  ...
  private async pushRealtime(
    ...
    await Promise.all(
      inAppRecipients.map((userId) =>
        this.notificationsService.invalidateUnreadCount(userId),
      ),
    );
  }
