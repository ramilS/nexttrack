  ...
  implements OnModuleInit
{
  ...
  async onModuleInit() {
    ...
    this.logger.log(
      `Notification jobs scheduled (due-date: ${this.config.dueDateCheckCron}, cleanup: ${CLEANUP_CRON})`,
    );
  }
  ...
  private async checkDueDates() {
    ...
    this.logger.log(`Due date check: ${issues.length} issues notified`);
  }
  ...
  private async cleanup() {
    ...
    this.logger.log(
      `Cleanup: ${deletedNotifs} notifications, ${deletedProcessed} processed outbox, ${deletedFailed} failed outbox, ${deletedKeys} idempotency keys`,
    );
  }
}
