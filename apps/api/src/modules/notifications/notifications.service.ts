...
export class NotificationsService {
  ...
  async getUnreadCount(userId: string): Promise<number> {
    ...
    await this.valkey.set(cacheKey, String(count), this.config.unreadCountCacheTtlSeconds);
    return count;
  }
  ...
  async markRead(userId: string, notificationIds: string[]) {
    ...
    this.logger.log('Notifications marked read', {
      userId,
  ...
  async markAllRead(userId: string) {
    ...
    await this.invalidateUnreadCount(userId);
    this.logger.log('All notifications marked read', { userId });
  }
  ...
  async remove(userId: string, notificationId: string) {
    ...
    this.logger.log('Notification deleted', { userId, notificationId });
  }

  async invalidateUnreadCount(userId: string) {
    await this.valkey.del(`notifications:unread:${userId}`);
  }
}
