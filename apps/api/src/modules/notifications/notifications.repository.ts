...
import { EmailMode, Notification, NotificationType, Prisma } from '@prisma/client';
...
// The list response (toDto) reads only scalar columns — issueId/projectId are
// already on the row — so findPage fetches the bare notification with no
// relation includes.
...
export class NotificationsRepository {
  ...
  /**
   * Bumps the group counter on every given notification in one statement using
   * an atomic `increment` (so concurrent dispatches can't clobber each other's
   * count) and refreshes the shared payload + updatedAt.
   */
  async incrementGroupedNotifications(
