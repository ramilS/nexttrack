...
export class MetricsCollectorsService {
  private readonly logger = new AppLogger(MetricsCollectorsService.name);
    ...
    new Gauge({
      ...
      async collect() {
        try {
          ...
          logger.warn('bullmq_queue_jobs collection failed', {
    ...
    new Gauge({
      ...
      async collect() {
        try {
          ...
          logger.warn('outbox_events collection failed', {
            error: (err as Error).message,
          });
        }
      },
