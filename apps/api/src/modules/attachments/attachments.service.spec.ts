...
describe('AttachmentsService', () => {
  ...
  describe('upload', () => {
    const file = {
      ...
      mimetype: 'image/png',
      size: 1024,
      // Valid PNG magic bytes so the declared image type matches the content.
    ...
    it('rejects active content disguised under an image MIME type', async () => {
      issuesRepo.findProjectIdById.mockResolvedValue('proj-1');
      attachmentsRepo.countActiveByIssue.mockResolvedValue(0);

      // Declared image/png, but the bytes are an HTML/script payload.
      const spoofed = {
        ...file,
      ...
      await expect(service.upload('issue-1', [spoofed], 'user-1')).rejects.toThrow(
        ValidationError,
      );
      expect(storage.uploadBuffer).not.toHaveBeenCalled();
    });

    it('rejects image/svg+xml (active content, removed from the allow-list)', async () => {
      issuesRepo.findProjectIdById.mockResolvedValue('proj-1');
      attachmentsRepo.countActiveByIssue.mockResolvedValue(0);

      const svg = { ...file, originalname: 'x.svg', mimetype: 'image/svg+xml' };

      await expect(service.upload('issue-1', [svg], 'user-1')).rejects.toThrow(
        ValidationError,
      );
    });
  ...
  describe('getDownloadUrl', () => {
    it('should return presigned URL for project member', async () => {
      ...
      const result = await service.getDownloadUrl('issue-1', 'att-1', 'user-1');
    ...
    it('should throw NotFoundError for missing attachment', async () => {
      ...
      await expect(service.getDownloadUrl('issue-1', 'missing', 'user-1')).rejects.toThrow(NotFoundError);
    });

    it('should throw NotFoundError when the attachment belongs to another issue (IDOR)', async () => {
      attachmentsRepo.findActiveById.mockResolvedValue(baseRaw); // issueId: 'issue-1'

      await expect(
        service.getDownloadUrl('other-issue', 'att-1', 'user-1'),
      ).rejects.toThrow(NotFoundError);
      expect(attachmentsRepo.findIssueProjectId).not.toHaveBeenCalled();
    });
  });
  ...
  describe('softDelete', () => {
    it('should soft-delete by uploader', async () => {
      ...
      await service.softDelete('issue-1', 'att-1', 'user-1', false);
    ...
    it('should allow admin to delete any attachment', async () => {
      ...
      await service.softDelete('issue-1', 'att-1', 'other-user', true);
    ...
    it('should throw PermissionDeniedError for non-uploader non-admin', async () => {
      ...
      await expect(service.softDelete('issue-1', 'att-1', 'other-user', false)).rejects.toThrow(PermissionDeniedError);
    ...
    it('should throw NotFoundError for missing attachment', async () => {
      ...
      await expect(service.softDelete('issue-1', 'missing', 'user-1', false)).rejects.toThrow(NotFoundError);
    });
    ...
    it('should throw NotFoundError when the attachment belongs to another issue (IDOR)', async () => {
      ...
      await expect(
        service.softDelete('other-issue', 'att-1', 'user-1', false),
      ).rejects.toThrow(NotFoundError);
      expect(attachmentsRepo.softDelete).not.toHaveBeenCalled();
    });
  });
