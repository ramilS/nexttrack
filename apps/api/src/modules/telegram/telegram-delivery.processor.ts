...
export class TelegramDeliveryProcessor extends WorkerHost {
  private readonly logger = new AppLogger(TelegramDeliveryProcessor.name);
  ...
  async process(job: Job) {
    ...
    this.logger.log('Telegram delivery started', {
      outboxEventId,
      telegramConfigId,
      eventType,
    ...
    try {
      ...
      if (!tgConfig) {
        this.logger.warn('Telegram config not found, skipping', {
          outboxEventId,
          telegramConfigId,
        });
      ...
      if (response.ok) {
        ...
        if (response.status === 429 && body.parameters?.retry_after) {
          ...
          this.logger.warn('Telegram rate limited, rescheduling', {
            outboxEventId,
            telegramConfigId,
            status: response.status,
      ...
      this.logger.error('Telegram delivery failed', error, {
        outboxEventId,
        telegramConfigId,
        eventType,
        attempt: job.attemptsMade + 1,
      ...
      if (tgConfig) {
        ...
        if (shouldDisable) {
          this.logger.warn('Telegram config auto-disabled after consecutive failures', {
            telegramConfigId,
            consecutiveFailures: failures,
          });
        }
