...
interface WidgetContext {
  ...
  /**
   * Member project ids, pre-resolved once per dashboard load and shared across
   * all widgets so each widget doesn't re-query them. Undefined for the
   * single-widget path, where {@link resolveProjectIds} resolves lazily.
   */
  projectIds?: string[];
}
...
export class WidgetDataService {
  private readonly logger = new AppLogger(WidgetDataService.name);

  constructor(
    private repo: DashboardsRepository,
    private reporting: DashboardReportingRepository,
  ) {}
  ...
  async getAllWidgetData(
    ...
    // Resolve the member project ids once and share them across every widget
    // in this dashboard, instead of each widget re-querying the same set.
    const projectIds = await this.reporting.findUserMemberProjectIds(userId);
    ...
    const entries = await Promise.all(
      dashboard.widgets.map(async (widget) => {
        ...
        try {
          ...
          this.logger.warn('Failed to fetch widget data', {
            dashboardId,
            widgetId: widget.id,
            type: widget.type,
  ...
  private async resolveProjectIds(ctx: WidgetContext): Promise<string[]> {
    return ctx.projectIds ?? this.reporting.findUserMemberProjectIds(ctx.userId);
  }
  ...
  private async getWatchedIssues(ctx: WidgetContext): Promise<IssueListWidgetData> {
    const issueIds = await this.reporting.findWatchedIssueIds(ctx.userId, 20);
  ...
  private async getOverdueIssues(ctx: WidgetContext): Promise<OverdueIssuesWidgetData> {
    const projectIds = await this.resolveProjectIds(ctx);
    const rows = await this.reporting.findOverdueIssues(projectIds, new Date(), 20);
    return {
      items: rows.map((r) => ({
        id: r.id,
  ...
  private async getRecentActivity(ctx: WidgetContext): Promise<RecentActivityWidgetData> {
    const projectIds = await this.resolveProjectIds(ctx);
    const rows = await this.reporting.findRecentActivities(projectIds, 15);
    return {
      items: rows.map((a) => ({
        id: a.id,
        type: a.type,
  ...
  private async getProjectProgress(ctx: WidgetContext): Promise<ProjectProgressWidgetData> {
    const projectIds = await this.resolveProjectIds(ctx);
    const projects = await this.reporting.findProjectsForProgress(projectIds);
    ...
    const countByStatus = await this.reporting.countResolvedIssuesByStatus(
  ...
  private async getTimeSpentToday(ctx: WidgetContext): Promise<TimeSpentTodayWidgetData> {
    ...
    const timeLogs = await this.reporting.findTimeLogsForUserBetween(
      ctx.userId,
      todayStart,
      todayEnd,
    );
  ...
  private async getIssuesByStatus(ctx: WidgetContext): Promise<IssuesByStatusWidgetData> {
    ...
    const groups = await this.reporting.groupIssuesByStatus(projectIds);
  ...
  private async getIssuesByPriority(ctx: WidgetContext): Promise<LabelCountWidgetData> {
    const projectIds = await this.resolveProjectIds(ctx);
    const groups = await this.reporting.groupIssuesByPriority(projectIds);
    return {
  ...
  private async getIssuesByType(ctx: WidgetContext): Promise<LabelCountWidgetData> {
    const projectIds = await this.resolveProjectIds(ctx);
    const groups = await this.reporting.groupIssuesByType(projectIds);
    return {
      items: groups.map((r) => ({ name: r.key, count: r.count })),
    };
  }
  ...
  private async getSprintBurndown(ctx: WidgetContext): Promise<SprintBurndownWidgetData> {
    const projectIds = await this.resolveProjectIds(ctx);
    const activeSprint = await this.reporting.findActiveSprintForProjects(projectIds);
    ...
    const resolvedByDay = await this.reporting.findResolvedByDayForSprint(
      activeSprint.id,
      effectiveEnd,
    );
  ...
  private async getCfdMini(ctx: WidgetContext): Promise<CfdMiniWidgetData> {
    ...
    const rows = await this.reporting.findCfdDailyCounts(
      projectIds,
      startDate,
      endDate,
    );
    ...
    // Resolve each status name to its (possibly several) status ids once,
    // rather than re-scanning statusMap for every day × series combination.
    const statusIdsByName = new Map<string, string[]>();
    for (const [id, ws] of statusMap) {
      const ids = statusIdsByName.get(ws.name);
      if (ids) ids.push(id);
      else statusIdsByName.set(ws.name, [id]);
    }
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
      ...
      for (const s of series) {
        const matchingIds = statusIdsByName.get(s.statusName) ?? [];
  ...
  private async getVelocityMini(ctx: WidgetContext): Promise<VelocityMiniWidgetData> {
    const projectIds = await this.resolveProjectIds(ctx);
    const closedSprints = await this.reporting.findClosedSprintsForProjects(
      projectIds,
      6,
    );
  ...
  private async findUserIssues(
    ...
    const rows = await this.reporting.findIssueList(where, 20);
  ...
  private async buildStatusMap(
    ...
    const workflows = await this.reporting.findWorkflowStatusBlobs(projectIds);
