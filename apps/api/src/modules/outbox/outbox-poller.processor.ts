  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  async onModuleInit() {
    ...
    this.logger.log(
      `Outbox poller scheduled every ${this.config.pollIntervalMs}ms`,
    );
  }
  ...
  async process() {
    ...
    if (claimed > 0) {
      this.logger.debug(`Outbox poll: claimed ${claimed} event(s)`);
    }
  }
  ...
  async pollOnce(): Promise<number> {
    ...
    for (const event of claimed) {
      try {
        ...
        this.logger.error(`Failed to enqueue outbox event ${event.id}`, error);
