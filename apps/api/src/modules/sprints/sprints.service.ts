...
export class SprintsService {
  ...
  async create(boardId: string, dto: CreateSprintInput): Promise<Sprint> {
    ...
    return this.sprintsRepo.create({
      boardId,
      name: dto.name,
      goal: dto.goal ?? null,
    ...
    const result = await this.txService.run(async (tx) => {
      ...
      if (incomplete.length > 0) {
        if (dto.incompleteIssuesAction === "MOVE_TO_BACKLOG") {
          ...
          const nextTotal = await this.issuesRepo.countActiveBySprint(
            dto.nextSprintId,
            tx,
          );
          const nextCompleted = await this.issuesRepo.countResolvedBySprint(
            dto.nextSprintId,
            tx,
          );
  ...
  private async recalculateCounters(sprintId: string) {
    const total = await this.issuesRepo.countActiveBySprint(sprintId);
    const completed = await this.issuesRepo.countResolvedBySprint(sprintId);
