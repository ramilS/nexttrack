...
export function buildDocUpdateProposal(overrides?: Record<string, unknown>) {
  return {
    id: nextId(),
    projectId: nextId(),
    sourceIssueId: nextId(),
    docIssueId: nextId(),
    targetArticleId: nextId(),
    proposedTitle: 'Updated article',
    proposedContent: { type: 'doc', content: [] },
    rationale: 'Issue resolution changed documented behavior.',
    status: DocUpdateProposalStatus.PENDING,
    baseArticleSha: 'sha-base',
