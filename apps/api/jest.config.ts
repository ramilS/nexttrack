...
const config: Config = {
  ...
  // Ratchet floor at the current unit level — not 80%, which is met by
  // unit ∪ integration (a separate run), never unit alone. Locks in today's
  // coverage so it can't silently rot; raise as unit tests are added.
  coverageThreshold: {
    global: { statements: 55, branches: 48, functions: 43, lines: 55 },
  },
