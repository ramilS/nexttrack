...
/**
 * Upper bound on a report's date span. The report query loads every matching
 * TimeLog into memory to aggregate in JS, so an unbounded range is a
 * memory/payload risk — cap it at the boundary.
 */
export const MAX_REPORT_RANGE_DAYS = 366;
const MS_PER_DAY = 86_400_000;

export const timeReportQuerySchema = timeReportQueryBaseSchema
  ...
  .refine(
    (data) =>
      (Date.parse(data.dateTo) - Date.parse(data.dateFrom)) / MS_PER_DAY <=
      MAX_REPORT_RANGE_DAYS,
    {
      message: `Report range must not exceed ${MAX_REPORT_RANGE_DAYS} days`,
      path: ['dateTo'],
    },
  );
