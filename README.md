...
- **Real-Time Updates** -- Socket.IO with Redis adapter for live collaboration
...
| Redis 7 (ioredis) | TanStack Query 5 |
...
This starts PostgreSQL, Redis, Elasticsearch, MinIO, and Mailhog.
...
├── infra/
│   ├── docker-compose.yml    Dev services (postgres, redis, es, minio, mailhog)
...
API integration tests use Testcontainers for real PostgreSQL + Redis:
...
1. `global-setup.ts` starts PostgreSQL 16 + Redis 7 containers, pushes Prisma schema, seeds data, then starts NestJS API + Next.js on random ports
